# ============================================================================
# ZSH Configuration — improved, diffed against github.com/radleylewis/zsh
# ============================================================================
...
# ----------------------------------------------------------------------------
# Kept: zinit's turbo/wait scheduling. This is strictly better for startup
# latency than the reference repo's synchronous `git clone + source` plugin
# manager — every plugin there blocks the prompt. Do not trade this away.
...
#
# ADDED vs. previous version: zsh-users/zsh-history-substring-search.
# This is the one plugin the reference config has that yours didn't —
# it makes Up/Down filter history by the substring already typed on the
# line, rather than plain chronological recall. Bound inside
# zvm_after_init below (zsh-vi-mode wipes bindkeys on init, same reason
# magic-space is bound there).
...
function zvm_after_init() {
  bindkey ' ' magic-space
  # History substring search — Up/Down filtered by what's already typed.
  bindkey '^[[A' history-substring-search-up
  bindkey '^[[B' history-substring-search-down
  # Ctrl+Right / Ctrl+Left — word-wise cursor movement (from reference repo).
...
# `menu no` (not the reference repo's `menu select`) is required for
# fzf-tab to intercept TAB with its fuzzy UI — menu select fights it and
# reintroduces the built-in menu. Keep this as-is; it is not a downgrade,
# it's a hard dependency of a plugin the reference config doesn't have.
...
# Prompt
# ----------------------------------------------------------------------------
# Kept oh-my-posh (already configured, themed, and working) over the
# reference repo's starship. Both are fast, single-binary, async-safe
# prompts — switching would be a lateral move with zero functional gain
# and would cost you re-theming. Not adopted.
...
# Keybinds
# ----------------------------------------------------------------------------
# (magic-space, history-substring-search, and Ctrl+arrows are (re)bound in
# zvm_after_init above, since vi-mode wipes bindings set before it inits.)
...
setopt HIST_EXPIRE_DUPS_FIRST  # ADDED — evict dupes before unique entries when HISTFILE hits SAVEHIST
...
# UNCHANGED, but flagged: this file is a vendored, hand-committed copy of
# fzf's upstream shell/key-bindings.zsh + completion.zsh (0.x-generation
# fzf, confirmed from its content). The reference repo instead does a
# cascaded `[[ -f /usr/share/fzf/key-bindings.zsh ]] && source ...` across
# distro paths, so it always tracks whatever fzf version pacman/apt/brew
# installed — zero maintenance, no drift. See "Recommended Changes" for
# why this is worth adopting; not changed here automatically because it
# alters your dotfiles' file layout and I don't want to silently orphan
# $HOME/dotfiles/zsh/fzf.zsh without your say-so.
...
# ADDED — Ctrl+F fzf file picker excluding hidden files/.git/node_modules.
# Complements the existing Ctrl+T (fzf.zsh, includes hidden files). Both
# widgets now coexist: Ctrl+T = everything, Ctrl+F = clean/visible-only.
_fzf_file_no_hidden() {
  local file
