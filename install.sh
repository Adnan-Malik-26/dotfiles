...
if [[ -d "$FIREFOX_PROFILE_DIR" ]]; then
  ...
  if [[ -n "$PROFILE" ]]; then
    ...
    create_symlink "$DOTFILES_DIR/firefox/chrome/userChrome.css" "$PROFILE/chrome/userChrome.css"
    create_symlink "$DOTFILES_DIR/firefox/chrome/userContent.css" "$PROFILE/chrome/userContent.css"
    create_symlink "$DOTFILES_DIR/firefox/chrome/user.js" "$PROFILE/user.js"
    # Symlink the entire theme directory
    create_symlink "$DOTFILES_DIR/firefox/chrome/theme" "$PROFILE/chrome/theme"
...
3. ${YELLOW}Tmux:${NC}
   - Install TPM (Tmux Plugin Manager) plugins: Press prefix + I
