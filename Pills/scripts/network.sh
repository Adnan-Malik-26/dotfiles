...
TIMEOUT=5
  ...
  if [[ $status == 'disabled' ]]; then
    ...
    notify-send -e -h string:x-canonical-private-synchronous:osd \
      'Wi-Fi Enabled' -i 'network-wireless-on' -r 1125
  fi
}
...
get-network-list() {
  ...
  if [[ -z $networks ]]; then
    notify-send -h string:x-canonical-private-synchronous:osd 'Wi-Fi' 'No networks found' -i 'package-broken'
    return 1
  fi
}
...
select-network() {
  ...
  if [[ -z $bssid ]]; then
    ...
    notify-send -h string:x-canonical-private-synchronous:osd 'Wi-Fi' 'Already connected to this network' \
      -i 'package-install'
    return 1
  fi
}
...
connect-to-network() {
  ...
  if nmcli --ask device wifi connect "$bssid"; then
    notify-send -h string:x-canonical-private-synchronous:osd 'Wi-Fi' 'Successfully connected' -i 'package-install'
  else
    notify-send -h string:x-canonical-private-synchronous:osd 'Wi-Fi' 'Failed to connect' -i 'package-purge'
  fi
}
