...
TIMEOUT=5
HELPER=$(command -v yay trizen pikaur paru pakku pacaur aurman aura | head -n 1 | xargs -- basename)
...
check-updates() {
  ...
  aur=0
  if [[ -n $HELPER ]]; then
    aur=$(timeout $TIMEOUT "$HELPER" -Quaq 2>/dev/null | wc -l) || aur=0
  fi
  total=$((repo + aur))
}
...
display-tooltip() {
  ...
  if ((total == 0)); then
    echo "{ \"text\": \"󰸟 0\", \"tooltip\": \"No updates available\" }"
  else
